...
g branch atras "$A"; bloquea "push --force de un ancestro sobre la rama remota (rebobina) → 2, no es 'sin cambios'" "$R" "git push --force origin atras:ya"
bloquea "+refspec que rebobina → 2" "$R" "git push origin +atras:ya"
pasa "sin --force el mismo sha ya en el remoto sigue pasando (git lo rechazaría como non-ff)" "$R" "git push origin ya"
...
json=$(ev "$R" "git push origin main"); err=$(printf '%s' "$json" | PATH=/nonexistent KIT_GATE_LEDGER="$LEDGER" "$BASH" "$HOOK" 2>&1 >/dev/null); rc=$?; [ $rc -eq 0 ] && grep -q '"motivo":"sin-python3"' "$LEDGER" && ok "sin python3 en el PATH → 0 y fila error-hook (sin-python3) escrita con builtins" || falla "sin python3: rc=$rc ledger=$(tail -1 "$LEDGER")"
