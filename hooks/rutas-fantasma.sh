...
# de verdad tienes /home/user, el hook no estorba una lectura legítima. Exit 2 =
# bloquear (stderr al modelo, con el cwd real). Si el JSON no se puede leer o
# falta python3, deja pasar: bloquear toda lectura sería peor que un error de ruta.
# Prefijos configurables con RUTAS_FANTASMA (separados por espacio, sin espacios
# dentro); lo usa la prueba. Para apagarlo, ver hooks/README.md.
set -uo pipefail
PREFIJOS="${RUTAS_FANTASMA:-/home/user /mnt/user-data /repo}"
input=$(cat)
# Prefiltro barato: si el JSON crudo no menciona ningún prefijo ni una ruta "/"
# a secas, no arranca python (es el 99% de las llamadas; ~3 ms en vez de ~20).
hit=0
for p in $PREFIJOS; do case "$input" in *"$p"*) hit=1 ;; esac; done
case "$input" in *'"file_path":"/"'*|*'"file_path": "/"'*) hit=1 ;; esac
[ "$hit" -eq 0 ] && exit 0
