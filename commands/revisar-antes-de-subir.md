...
bash "${SELLO:-$HOME/.claude/scripts/sello-push.sh}" revisar     # en un Bash con timeout: 600000
```

## Pasos

1. **Revisar.** Corre `revisar` en un Bash con `timeout: 600000` (corre las pruebas del
   proyecto en un worktree desechable y luego el revisor; puede tardar varios minutos). Si lo
   lanzas sin timeout, el hook lo devuelve con la instrucción.
